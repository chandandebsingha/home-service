  ...
  static async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (booking.status !== 'completed') {
        res.status(400).json({ success: false, error: 'You can only review completed bookings' });
        return;
      }

  // Ensure no duplicate review for this booking & target
  const existing = await ReviewService.getByBookingAndTarget(parsedBookingId, 'customer');
      if (existing) {
        res.status(409).json({ success: false, error: 'Review already exists for this booking' });
        return;
      }
      ...
      const createdUnified = await ReviewService.create({
        ...
        providerId: (await ServiceService.getById(booking.serviceId))?.providerId ?? null as any,
