...
class ProviderProfileService {
    static async create(newProfile) {
        const [created] = await db_1.db
            .insert(schema_1.providerProfiles)
            .values(newProfile)
            .returning();
        return created;
    }
    static async getByUserId(userId) {
        const rows = await db_1.db
            ...
            addressId: schema_1.providerProfiles.addressId,
    ...
    static async update(id, updates) {
        const [updated] = await db_1.db
            .update(schema_1.providerProfiles)
            .set(updates)
            .where((0, drizzle_orm_1.eq)(schema_1.providerProfiles.id, id))
            .returning();
        return updated;
    }
    ...
    static async verifyAndPromote(profileId) {
        return db_1.db.transaction(async (tx) => {
            const existing = await tx
                ...
                .from(schema_1.providerProfiles)
                .where((0, drizzle_orm_1.eq)(schema_1.providerProfiles.id, profileId))
                .limit(1);
            ...
            await tx
                ...
                .set({ role: "partner" })
                .where((0, drizzle_orm_1.eq)(schema_1.users.id, profile.userId));
