...
export const getApiUrl = (endpoint: string): string => `${API.BASE_URL}${endpoint}`
...
export interface AdminStatsResponse {
  ...
  recent: {
    bookings: Array<{ id: number; date: string; time: string; price: number; createdAt?: string }>
    services: Array<{ id: number; name: string; price: number; serviceType?: string; createdAt?: string }>
  }
}

export async function getAdminStats(token?: string): Promise<ApiResponse<AdminStatsResponse>> {
...
// Auth API functions
export async function apiPost<T>(endpoint: string, data: any, token?: string): Promise<ApiResponse<T>> {
  try {
    const res = await fetch(getApiUrl(endpoint), {
      method: "POST",
      headers: {
...
export async function getAllServices(token?: string): Promise<ApiResponse<Service[]>> {
  return apiGet(API.ENDPOINTS.SERVICES.LIST, token)
}

export async function getService(id: number, token?: string): Promise<ApiResponse<Service>> {
...
export async function getMyServices(token: string): Promise<ApiResponse<Service[]>> {
  return apiGet(API.ENDPOINTS.PROVIDER.SERVICES.LIST, token)
}

export async function createMyService(token: string, serviceData: CreateServiceRequest): Promise<ApiResponse<Service>> {
  return apiPost(API.ENDPOINTS.PROVIDER.SERVICES.CREATE, serviceData, token)
}

export async function updateMyService(token: string, serviceId: number, serviceData: Partial<CreateServiceRequest>): Promise<ApiResponse<Service>> {
  return apiPost(API.ENDPOINTS.PROVIDER.SERVICES.UPDATE(serviceId), serviceData, token)
}

export async function deleteMyService(token: string, serviceId: number): Promise<ApiResponse> {
...
export async function updateBookingStatus(token: string, bookingId: number, status: string): Promise<ApiResponse<Booking>> {
  return apiPost(API.ENDPOINTS.PROVIDER.BOOKINGS.UPDATE_STATUS(bookingId), { status }, token)
}
