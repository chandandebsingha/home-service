...
export default function PartnerBookingsScreen() {
  ...
  const loadBookings = useCallback(async () => {
    try {
      ...
      // partners should call partner-specific endpoint
      const res = await apiService.listPartnerBookings(accessToken);
      if (res.success && res.data) {
        setBookings(res.data);
      } else {
        console.error("listPartnerBookings failed", res);
        Alert.alert(
          "Error",
          res.error || res.message || "Failed to load bookings"
        );
      }
    } catch (err: any) {
