...
import { apiService, Service, ServiceType } from "../../src/services/api";
...
export default function CategoryScreen() {
	...
	const categoryId = Number.isNaN(Number(categoryIdParam))
		? undefined
	...
	useEffect(() => {
		...
		(async () => {
			...
			const [servicesRes, typesRes] = await Promise.all([
				apiService.listServices(100, 0, { categoryId }),
				apiService.listServiceTypes(categoryId),
			]);
			if (!mounted) return;
			if (servicesRes.success && servicesRes.data) {
				setServices(servicesRes.data);
			} else {
				setError(servicesRes.error || "Failed to load services");
			}

			if (typesRes.success && typesRes.data) {
				setTypes(typesRes.data);
			} else {
				// silently ignore types error for now
				// setError(typesRes.error || 'Failed to load types');
			}
			setLoading(false);
		})();
		return () => {
						...
						<View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
							{types.map((t) => (
								<TouchableOpacity
									key={t.id}
									onPress={() =>
										router.push(`/category/${categoryId}/types/${t.id}`)
									}
									style={{
										paddingVertical: 8,
								...
								>
									<Text style={{ color: "#3730a3" }}>{t.name}</Text>
								</TouchableOpacity>
							))}
						</View>
					</View>
